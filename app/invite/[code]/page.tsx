...
export default function InvitePage({ params }: { params: { code: string } }) {
    ...
    if (authenticated && user) {
      ...
      const fetchOrCreateUser = async () => {
        try {
          ...
          const res = await fetch('/api/demo/users/me', {
            method: 'POST',
  ...
  const joinChannel = async () => {
    ...
    if (!userId || !inviteDetails?.channel?.id) {
      showToast('Unable to join channel at this time', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json();
        showToast(errorData.error || 'Failed to join channel', 'error');
      }
    } catch (err) {
